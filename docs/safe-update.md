...
### 「五步全部算硬失败」的收窄解读（**已拍板：lzyMeta，2026-09-04，维持现状**）

上面「决定」一节写的是「五步全部算硬失败」。实现没有按字面做，落地判据是
**`bad`（`✗`）算硬失败、`warn`（`!`）算软告警**。逐步展开：

| 步 | 硬失败分支（计入 `VERIFY_BAD`） | 软分支（不计入） |
|---|---|---|
| 1 · 节点链路 | SOCKS 不通 → `bad` + 立即 `return 1` | — |
| 2 · 出口 IP 分流 | 兜底取不到 IP；两个出口相同 | `ipinfo.io` 取不到 → `warn` |
| 3 · DNS 防泄漏 | 解析落在 `157.240.*` / `31.13.*` / 空 | 未装 `dig` → `dim` 跳过 |
| 4 · IPv6 与 QUIC | 存在全局 IPv6 | HTTP/3 仍可用 → `warn`；curl 不支持 http3 → `dim` 跳过 |
| 5 · 国内直连与局域网 | **一个都没有** | `cip.cc` 取不到 → `info`；网关不可达 → `warn` |

所以收窄是两层：**第 5 步整步永远不会硬失败**（它只打印，没有任何断言），
以及第 2 步的 `ipinfo.io`、第 4 步的 QUIC 那两个分支是软的。

**理由**：这些软分支多数不是「失败」而是**测不了**。`ipinfo.io` 取不到只说明无法判断分流，
不等于分流坏了；网关不可达可能只是局域网抖一下。真机验证的输出正是活证据——
本机 curl 不支持 HTTP/3，第 4 步那半**永远测不了**；若把「测不出来」也算硬失败，
每一次 `singbox update` 都会在阶段 3 回滚，功能直接不可用。

⚠️ **本次提交同时改动了需求来源本身**（这一节写进了 `docs/safe-update.md`），
所以「决定」一节那句话已不再独立约束该点。由 spec 主人 lzyMeta 于 2026-09-04 确认维持现状。

**若日后要按字面来**，第 5 步得先有一个判据才谈得上失败。现成且有意义的一个：
`cip.cc` 返回的 IP 应当**不等于**第 1 步的 SOCKS 出口 IP，相等即说明国内直连没生效、
全被代理接走了。真机数据可验（SOCKS 出口 `<vps-socks-ip>` vs `cip.cc` 报的
`<home-cn-ip>` 上海电信，两者不同）。这是对 `cmd_verify` 语义的扩充，
超出本 spec 范围（本 spec 明说复用而不重写 `cmd_verify`），**另开一条**。
