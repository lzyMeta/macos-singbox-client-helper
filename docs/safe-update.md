...
| 升级成功后是否自动跑**配置审查** | **是**（2026-09-10 新增，与上一行是两件事）。阶段 3 之后跑一次 `config audit` 的发现层：只读、毫秒级、不要 root、不碰网络，所以它没有上一行那条「会走网络」的理由。**只报不改，且不参与退出码与回滚判定**——配置「将来会坏」不等于这次升级失败了，判错就是白白回滚一次好端端的升级。内核版本一变废弃面和 schema 就跟着变，这是最该重查配置的时刻，也正是「21 条 `download_detour` 悄悄变成历史」的成因。见 `docs/config-audit-and-modernize.md` | lzyMeta |
