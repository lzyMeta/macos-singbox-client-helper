...
查最新版 → 确认 → 备份旧二进制 → 安装 → **用新内核校验当前配置** → 通过才重启，不通过**自动回滚**。

这个顺序是有意的：sing-box 迭代快，新版本可能移除你正在用的字段（`domain_strategy` 在 1.14 被移除、`independent_cache` 在 1.14 废弃）。先校验后重启，避免升级完才发现起不来。
