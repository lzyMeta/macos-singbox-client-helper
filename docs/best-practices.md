...
- [9. 脚本的设计取舍](#9-脚本的设计取舍)
  - [9.1 几条纪律](#91-几条纪律)
  - [9.2 verify 的两档退出码](#92-verify-的两档退出码)
  - [9.3 日志为什么只能原地截断](#93-日志为什么只能原地截断)
  - [9.4 DNS 备份的归一化](#94-dns-备份的归一化)
  - [9.5 edit 的编辑器回退](#95-edit-的编辑器回退)
  - [9.6 update 的沙箱与完整性校验](#96-update-的沙箱与完整性校验)
  - [9.7 镜像回退](#97-镜像回退)
  - [9.8 架构判定](#98-架构判定)
...
操作手册（`docs/manual-*.md`）只讲怎么做；这一节讲脚本为什么那样做。

### 9.1 几条纪律

* **绝不 `kill -9`。** 正常退出时 sing-box 会拆掉 utun 并还原路由表；强杀留下残留路由，症状是断网且看不出原因。脚本一律 SIGTERM、等 8 秒，超时只警告。
* **用 `bootstrap` / `bootout`，不用 `load` / `unload`。** 后者是遗留接口，`Load failed: 5: Input/output error` 几乎不给线索。
* **配置永远不被就地破坏。** `install` 用工作副本，`edit` 校验不过不写入且保留修改，覆盖前一定备份、自动保留 10 份。
* **并发保护。** 锁目录防两个实例同时改配置或加载服务，属于已死进程的锁自动清理。
* **只读命令不要 sudo**（`verify` / `syscheck` / `rules`），**非 tty 自动取默认**，`--help` 在任何平台都能看（平台检查放在参数解析之后）。

### 9.2 verify 的两档退出码

分档的判据只有一条：**回滚到旧内核能不能把它换回来。** 能的是链路档（退 `1`）：SOCKS 不通、取不到出口 IP、两个出口相同、有全局 IPv6；不能的是策略档（退 `2`）：DNS、QUIC、国内直连、网关、参照站、废弃字段——这些是路由策略与环境的问题，换内核一个字都改不了。`update` 因此只认 `1` 才回滚。

**第 2 步「两个出口 IP 不同」是全脚本最关键的一项。** 日志只能证明连接派给了 `vpsre` 出站，证明不了 `vpsre` 的出口真是住宅 IP——那段中转在服务端，客户端看不见。中转挂了日志照样打 `[vpsre]`，出口却已变回机房 IP。

六步里没有静默跳过：`dig` 不可用降级到 `host` → `dscacheutil` → `python3`，四级全废才报「无解析手段」，与「疑似污染」分开报；QUIC 不依赖 `curl --http3`（macOS 自带的 curl 没编 HTTP/3），改为直接发一个 QUIC 版本协商包看对端回不回。

### 9.3 日志为什么只能原地截断

plist 把 stdout / stderr 指向 `/var/log/sing-box.log` 与 `.err`，launchd 只管写，不轮转、不封顶，实测能涨到几百 MB。那个 fd 是 launchd 打开、`dup2` 到子进程 fd 2 上的：一旦换了 inode（`mv`、`rm` 后重建、任何 rename 式轮转），守护进程会一直往没有名字的旧文件里写——磁盘收不回来，新日志也看不到。`logs truncate` 用 `: > 文件`，inode 不变。

不配 `newsyslog`，因为它只会 rename + 新建，没有原地截断选项（`man newsyslog.conf` 的 flags 里没有一个是截断）。配上去等于装了一个看着在管日志、实际让守护进程往归档文件里写的东西。

### 9.4 DNS 备份的归一化

首次 `install` 改系统 DNS 之前，脚本把每个网络服务的原值记进 `~/.config/singbox/dns-backup`，还原时逐条写回。**备份里等于 `1.1.1.1` 的条目按 DHCP 处理**：如果装脚本之前就手动把 DNS 设成了 `1.1.1.1`，备份记下的就是它，照着回滚等于没还原。写备份和读备份两处都归一化。还原后回读实际生效值打印出来，避免「以为还原了其实没有」。

### 9.5 edit 的编辑器回退

优先级 `--editor` > 已保存偏好 > `$EDITOR` > `vi`，任一级不可用就降级并说明原因；已保存的编辑器被卸载会自动清除偏好。**只有「显式指定且确实可用」才写入偏好**——回退得到的结果不记住，免得一次意外把错的编辑器固化下来。验证只看命令的第一个词，所以 `code -w` 这类带参数的写法没问题。

### 9.6 update 的沙箱与完整性校验

`check -c` 校验的是语法与字段合法性，字段还在、语义变了它照样过；sing-box 迭代快（`1.11` 换 DNS 格式、`1.14` 移除 `domain_strategy`），静态校验通过但新内核起不来或代理坏了都真实发生过。所以阶段 1 要真的把新内核跑起来试一次，用**派生配置**：删掉 `tun` 入站（不然和现网撞设备）、`mixed` 端口换成从 `10900` 起探到的闲置端口、`cache_file.path` 指到临时目录。代价是沙箱验不到 TUN 与系统路由的回归，那类问题只能在阶段 2/3 暴露、靠回滚兜底。

上游 release 没有 checksum 文件，但 GitHub Releases API 的每个 asset 带 `digest`（`sha256:<hex>`）。下载完当场比对，对不上就删；取不到就 warn 后放行，不装作校验过。它能挡传输损坏与单个镜像投毒，挡不住「API 与文件出自同一个坏镜像」——它不是签名。架构那一步仍照验：sha256 证明不了「下对了平台」。

阶段 S（脚本自更新）**不许阻断内核升级**：取不到新版、下载失败、`bash -n` 不过，一律 warn 一句继续。远端版本相等或更低也不动——release 被回退时把用户降级，等于把修好的 bug 再装回去。替换 `/usr/local/bin/singbox` 用 `mv` 不用 `cp`：bash 边执行边按偏移量读脚本文件，`cp` 覆盖同一个 inode，正在跑的进程下一次读取会落在错误的偏移上。

`.prev` 只由 `update` 写、只保留一份。`install` 的临时回滚点放在临时目录——早先两者共用 `$BIN.prev`，`update` 成功后再跑一次 `install` 会把退路无声删掉。

### 9.7 镜像回退

前缀式镜像站更替频繁，所以不假定哪个一定可用：内置一份列表，每次先用 HEAD 探测（每源最多 6 秒、建连 4 秒），只有探通的才真正下载；下载中 20 秒低于 2 KB/s 判为卡住换下一个。成功的镜像记进 `~/.config/singbox/prefs`，但直连始终排最前，网络恢复自然走直连。版本号查询三层兜底：GitHub API → 镜像上的 API → 解析 `releases/latest` 的 302 跳转。

### 9.8 架构判定

判据是硬件（`sysctl -n hw.optional.arm64`），**不是 `uname -m`**——后者报的是当前进程的架构，在 Rosetta 方式打开的终端、x86_64 的 Homebrew bash 或 `arch -x86_64 bash` 里会说 `x86_64`。早先按它判断，会在 ARM 机器上装 Intel 内核，一个常驻的网络路径守护进程被塞进翻译层，而且 `update` 走同一个函数，会把这个错误一直续下去。检测到当前 shell 被翻译（`sysctl.proc_translated` = 1）时 `install` 会明说，并提示用 `arch -arm64 zsh` 开原生 shell。
